...
$work_question = "Какая задача стоит перед вами сегодня?\n";
$time_question = "Сколько примерно часов эта задача займет?\n";
...
while ($i < 3) {
    ...
    $time = "time$i";
    $$time = (int)readline($time_question);

    if (!$$time) {
        $$time = 0;
    }

    $total_string .= "- {$$work} ({$$time}ч)\n";
    $total_time += $$time;
